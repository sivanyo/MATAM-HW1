...
int main(){
    ...
    assert(MATAMAZOM_INVALID_AMOUNT==mtmNewProduct(mtm,1,"F",5.502,
            MATAMAZOM_HALF_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
    assert(MATAMAZOM_INVALID_AMOUNT==mtmNewProduct(mtm,1,"F",5.5070000000000001,
            MATAMAZOM_HALF_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
    assert(MATAMAZOM_INVALID_AMOUNT==mtmNewProduct(mtm,1,"F",5.4979999999999999,
            MATAMAZOM_HALF_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
    assert(MATAMAZOM_INVALID_AMOUNT==mtmNewProduct(mtm,1,"F",5.0070000000000001,
            MATAMAZOM_HALF_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
    assert(MATAMAZOM_INVALID_AMOUNT==mtmNewProduct(mtm,1,"F",5.9979999999999999,
            MATAMAZOM_HALF_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));

    //checking the MATAMAZOM_INVALID_AMOUNT case if you got the right limits
    // of amount that can be send :
    assert(MATAMAZOM_SUCCESS==mtmNewProduct(mtm,1,"First",5,
            MATAMAZOM_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
    assert(MATAMAZOM_SUCCESS==mtmNewProduct(mtm,2,"Second",3,
            MATAMAZOM_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
    assert(MATAMAZOM_SUCCESS==mtmNewProduct(mtm,5,"cat",5.5,
            MATAMAZOM_HALF_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
    assert(MATAMAZOM_SUCCESS==mtmNewProduct(mtm,4,"dog",7.5,
            MATAMAZOM_HALF_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
    assert(MATAMAZOM_SUCCESS==mtmNewProduct(mtm,8,"bed",9.5,
            MATAMAZOM_HALF_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
    assert(MATAMAZOM_SUCCESS==mtmNewProduct(mtm,9,"shite",9.5,
            MATAMAZOM_HALF_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
    assert(MATAMAZOM_SUCCESS==mtmNewProduct(mtm,7,"bed",10,
            MATAMAZOM_HALF_INTEGER_AMOUNT,&basePrice,copyDouble,freeDouble,simplePrice));
