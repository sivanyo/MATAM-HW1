...
testElement copyFunction (testElement Element){
    ...
    testElement newElement = malloc(sizeof(Element));
    if (newElement == NULL) {
        free(newElement);
        return NULL;
    }
...
bool asGetSizeCheck (ASElement element, int size) {
    if (asGetSize(element) != size) {
        return false;
    }
    return true;
}

bool asContainCheck (AmountSet set, ASElement element) {
...
bool asGetAmountCheck (AmountSet set, ASElement element, double *outAmount) {
    ...
    if(asChangeAmount(set, outAmount, amount1) == AS_SUCCESS) {
...
int main(){
    ...
    if (element==NULL){
        ...
        element->letter='a';
        element->number=1;
        RUN_TEST(!asContainCheck, set_test, element);
        asRegister(set_test,element);
